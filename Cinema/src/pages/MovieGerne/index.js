...
export default function MovieGerne() {
  ...
  console.log("selectedPhim.current", selectedPhim.current);
    ...
    <div style={{ height: "calc(100vh - 64px)", width: "100%", paddingBottom: "24px" }}>
      ...
      <Dialog open={openModal}>
        <DialogTitle onClose={() => setOpenModal(false)}>
          {selectedPhim?.current && selectedPhim?.current.id != ''
            ? `Sửa Thể Loại Phim: Mã ${selectedPhim?.current.id}`
            : "Thêm Thể Loại Phim"}
        </DialogTitle>
